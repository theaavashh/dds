...
export default function AdminLogin() {
  ...
  const [currentStep, setCurrentStep] = useState<'login' | 'otp'>('login'); // New state for multi-step
  ...
  // Handle first step: email and password submission
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/auth/verify-credentials`, {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      // If credentials are valid, send OTP and move to OTP step
      ...
      // Store email for OTP verification and move to OTP step
      setEmail(data.email);
      setCurrentStep('otp');
  ...
  // Handle second step: OTP verification
  const onOtpSubmit = async (data: OtpFormData) => {
    setIsLoading(true);

    try {
      // Verify OTP with the stored email
      const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:5000';
      ...
      // If OTP is valid, the auth cookie is set. We just need to refresh the user state.
      await refreshUser();
        ...
        <div className="bg-[#fffefe] shadow-lg rounded-xl p-8 border border-gray-200">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900 mb-2">
              {currentStep === 'login' ? 'Welcome back' : 'Verify OTP'}
            </h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 border rounded-lg text-lg ${errors.email
                    ? "border-red-300"
                    : "border-gray-300"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 py-2.5 pr-10 border rounded-lg text-lg ${errors.password
                      ? "border-red-300"
                      : "border-gray-300"
                      } bg-white text-gray-900 placeholder-gray-400`}
          ...
          {currentStep === 'otp' && (
            <form onSubmit={handleOtpSubmit(onOtpSubmit)} className="space-y-6">
              <div>
                <label htmlFor="otp" className="block text-xl font-medium text-black mb-2">
                  Enter OTP
                </label>

                {/* Six separate input fields for OTP */}
                <div className="flex justify-between gap-3">
                  {[0, 1, 2, 3, 4, 5].map((index) => (
                    <input
                      ...
                      inputMode="numeric"
                      maxLength={1}
                      className={`w-12 h-14 text-center text-xl border rounded-lg ${otpErrors.otp
                        ? "border-red-300"
                        : "border-gray-300"
                        } bg-white text-gray-900 placeholder-gray-400`}
                      placeholder=""
              ...
              <div className="flex space-x-3">
                ...
                <button
                  type="button"
                  onClick={() => {
                    // Collect values from all 6 inputs
                    const otpValue = [0, 1, 2, 3, 4, 5]
            ...
            <form onSubmit={handleForgotPasswordSubmit(onForgotPasswordSubmit)} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 border rounded-lg text-sm ${forgotPasswordErrors.email
                    ? "border-red-300"
                    : "border-gray-300"
                    } bg-white text-gray-900 placeholder-gray-400`}
                  placeholder="Enter email address"
