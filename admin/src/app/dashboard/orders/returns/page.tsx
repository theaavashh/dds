...
const reasonColors = {
  ...
  'not-as-described': 'bg-yellow-100 text-yellow-800',
  'changed-mind': 'bg-blue-100 text-blue-800',
  'damaged-shipping': 'bg-purple-100 text-purple-800',
...
export default function ReturnsPage() {
    ...
    const mockReturns: Return[] = [
      {
        ...
        total: 2500,
        status: 'approved',
        returnReason: 'Product arrived damaged and not working properly',
        returnDate: '2024-01-20',
        requestedDate: '2024-01-18',
        processedDate: '2024-01-21',
        refundAmount: 2500,
        refundMethod: 'Credit Card',
        notes: 'Customer provided photos of damaged product'
      },
      {
        id: '2',
        ...
        total: 800,
        status: 'pending',
        returnReason: 'Received wrong color - ordered black but got blue',
        returnDate: '2024-01-22',
        requestedDate: '2024-01-20',
        notes: 'Customer wants exchange for correct color'
      },
      {
        id: '3',
        ...
        total: 1200,
        status: 'rejected',
        returnReason: 'Product does not match description on website',
        returnDate: '2024-01-19',
        requestedDate: '2024-01-17',
        processedDate: '2024-01-20',
        notes: 'Product matches description - return rejected'
      }
    ];
  ...
  const handleBulkAction = (action: string) => {
    console.log(`Bulk action: ${action} on returns:`, selectedReturns);
  ...
  if (isLoading) {
    return (
      <DashboardLayout title="Returns" showBreadcrumb={true}>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredReturns.map((returnItem) => (
                  <tr key={returnItem.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${returnItem.items[0]?.reason ? reasonColors[returnItem.items[0].reason as keyof typeof reasonColors] || 'bg-gray-100 text-gray-800' : 'bg-gray-100 text-gray-800'}`}>
                        {getReasonText(returnItem.items[0]?.reason)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[returnItem.status]}`}>
                        {getStatusIcon(returnItem.status)}
                        <span className="ml-1 capitalize">{returnItem.status}</span>
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {returnItem.refundAmount ? formatPrice(returnItem.refundAmount) : 'N/A'}
                    </td>
